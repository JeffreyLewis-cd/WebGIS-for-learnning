...
! function(t, e) {
	...
	function r(t, e, r, n) {
		var o = s(r, n);
		return o.open("POST", t), void 0 !== n && null !== n && void 0 !== n.options && (o.timeout = n.options.timeout), o.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"), o.send(i(e)), o
	}

	function n(t, e, r, n) {
		var o = s(r, n);
		return o.open("GET", t + "?" + i(e), !0), void 0 !== n && null !== n && void 0 !== n.options && (o.timeout = n.options.timeout), o.send(null), o
	}
	...
	function o(t, e, r, n) {
		var o = i(e),
			u = s(r, n),
			l = (t + "?" + o).length;
		if (l <= 2e3 && it.cors ? u.open("GET", t + "?" + o) : l > 2e3 && it.cors && (u.open("POST", t), u.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8")), void 0 !== n && null !== n && void 0 !== n.options && (u.timeout = n.options.timeout), l <= 2e3 && it.cors) u.send(null);
		else {
			if (!(l > 2e3 && it.cors)) return l <= 2e3 && !it.cors ? a(t, e, r, n) : void P("a request to " + t + " was longer then 2000 characters and this browser cannot make a cross-domain post request. Please use a proxy http://esri.github.io/esri-leaflet/api-reference/request.html");
			u.send(o)
		}
		return u
	}
	...
	function M(t) {
		...
		return t instanceof e.LatLngBounds ? (i.geometry = I(t), i.geometryType = "esriGeometryEnvelope", i) : (t.getLatLng && (t = t.getLatLng()), t instanceof e.LatLng && (t = {
			type: "Point",
			coordinates: [t.lng, t.lat]
		}), t instanceof e.GeoJSON && (t = t.getLayers()[0].feature.geometry, i.geometry = x(t), i.geometryType = O(t.type)), t.toGeoJSON && (t = t.toGeoJSON()), "Feature" === t.type && (t = t.geometry), "Point" === t.type || "LineString" === t.type || "Polygon" === t.type || "MultiPolygon" === t.type ? (i.geometry = x(t), i.geometryType = O(t.type), i) : void P("invalid geometry passed to spatial query. Should be L.LatLng, L.LatLngBounds, L.Marker or a GeoJSON Point, Line, Polygon or MultiPolygon object"))
	}
	...
	function G(t) {
		...
		if (i && i.attributionControl && s) {
			...
			c.innerHTML = r, c.style.maxWidth = F(i), i.fire("attributionupdated", {
				attribution: r
			})
		}
	}
	...
	var ot = {
		...
		ut = e.Class.extend({
			...
			initialize: function(t) {
				if (t.request && t.options ? (this._service = t, e.Util.setOptions(this, t.options)) : (e.Util.setOptions(this, t), this.options.url = R(t.url)), this.params = e.Util.extend({}, this.params || {}), this.setters)
		...
		lt = ut.extend({
			...
			run: function(t, e) {
				return this._cleanParams(), this.options.isModern || C(this.options.url) ? (this.params.f = "geojson", this.request(function(i, s) {
			...
			_trapSQLerrors: function(t) {
				t && "400" === t.code && P("one common syntax error in query requests is encasing string values in double quotes instead of single quotes")
			},
		...
		dt = ct.extend({
			...
			_responseToGeoJSON: function(t) {
				...
				if (t.properties && t.properties.Values && (r.pixel.properties.values = t.properties.Values), i && i.features && (r.catalogItems = w(i), s && s.length === r.catalogItems.features.length))
		...
		mt = e.Evented.extend({
			...
			initialize: function(t) {
				t = t || {}, this._requestQueue = [], this._authenticating = !1, e.Util.setOptions(this, t), this.options.url = R(this.options.url)
			},
		...
		vt = e.TileLayer.extend({
          ...
          chinacolor: {
            urlTemplate: "http://cache1.arcgisonline.cn/ArcGIS/rest/services/ChinaOnlineStreetWarm/MapServer/tile/{z}/{y}/{x}",
            options: {
              minZoom: 1,
              maxZoom: 19
            }
          ...
          GaodeVec_A: {
			...
			onAdd: function(t) {
				k(t), "esri-labels" === this.options.pane && this._initPane(), this.options.attributionUrl && U(this.options.attributionUrl, t), t.on("moveend", G), e.TileLayer.prototype.onAdd.call(this, t)
			},
			onRemove: function(t) {
				...
				k(t), this._lodMap || this.metadata(function(e, i) {
					if (!e && i.spatialReference) {
						...
						if (!this.options.attribution && t.attributionControl && i.copyrightText && (this.options.attribution = i.copyrightText, t.attributionControl.addAttribution(this.getAttribution())), t.options.crs === L.CRS.EPSG3857 && 102100 === s || 3857 === s) {
							...
							this.fire("lodmap")
						} else proj4 || P("L.esri.TiledMapLayer is using a non-mercator spatial reference. Support may be available through Proj4Leaflet http://esri.github.io/esri-leaflet/examples/non-mercator-projection.html")
					}
		...
		xt = e.Layer.extend({
			...
			_renderImage: function(t, e, i) {
				if (this._map) {
					...
					s.once("error", r, this), s.once("load", n, this), this.fire("loading", {
						bounds: e
					})
				}
			},
		...
		St = xt.extend({
			...
			initialize: function(t) {
				t.url = R(t.url), this.service = W(t), this.service.addEventParent(this), e.Util.setOptions(this, t)
			},
		...
		At = xt.extend({
			...
			initialize: function(t) {
				t.url = R(t.url), this.service = Z(t), this.service.addEventParent(this), (t.proxy || t.token) && "json" !== t.f && (t.f = "json"), e.Util.setOptions(this, t)
			},
			...
			_getPopupData: function(t) {
				...
				if (s.simplify(this._map, .5), this.options.layers ? s.layers("visible:" + this.options.layers.join(",")) : s.layers("visible"), this.options.layerDefs && "string" != typeof this.options.layerDefs)
					for (var r in this.options.layerDefs) this.options.layerDefs.hasOwnProperty(r) && s.layerDef(r, this.options.layerDefs[r]);
			...
			_buildExportParams: function() {
				...
				return this.options.dynamicLayers && (e.dynamicLayers = this.options.dynamicLayers), this.options.layers && (e.layers = "show:" + this.options.layers.join(",")), this.options.layerDefs && (e.layerDefs = "string" == typeof this.options.layerDefs ? this.options.layerDefs : JSON.stringify(this.options.layerDefs)), this.options.timeOptions && (e.timeOptions = JSON.stringify(this.options.timeOptions)), this.options.from && this.options.to && (e.time = this.options.from.valueOf() + "," + this.options.to.valueOf()), this.service.options.token && (e.token = this.service.options.token), this.options.proxy && (e.proxy = this.options.proxy), this.options.disableCache && (e._ts = Date.now()), e
			},
			_requestExport: function(t, i) {
		...
		Tt = Y.Layer.extend({
			...
			onAdd: function(t) {
				this._map = t, this._update = Y.Util.throttle(this._update, this.options.updateInterval, this), this._reset(), this._update()
			},
			onRemove: function() {
			...
			_addCells: function(t) {
				...
				for (e = t.min.y; e <= t.max.y; e++)
					for (i = t.min.x; i <= t.max.x; i++) s = Y.point(i, e), s.z = o, this._isValidCell(s) && r.push(s);
				var a = r.length;
				if (0 !== a)
					for (this._cellsToLoad += a, this._cellsTotal += a, r.sort(function(t, e) {
			...
			_wrapCoords: function(t) {
				t.x = this._wrapLng ? Y.Util.wrapNum(t.x, this._wrapLng) : t.x, t.y = this._wrapLat ? Y.Util.wrapNum(t.y, this._wrapLat) : t.y
			},
			_getCellNumBounds: function() {
				var t = this._map.getPixelWorldBounds(),
					e = this._getCellSize();
				return t ? Y.bounds(t.min.divideBy(e).floor(), t.max.divideBy(e).ceil().subtract([1, 1])) : null
			}
		});
	...
	var It = Tt.extend({
			...
			initialize: function(t) {
				if (Tt.prototype.initialize.call(this, t), t.url = R(t.url), t = e.Util.setOptions(this, t), this.service = J(t), this.service.addEventParent(this), "*" !== this.options.fields[0]) {
					for (var i = !1, s = 0; s < this.options.fields.length; s++) this.options.fields[s].match(/^(OBJECTID|FID|OID|ID)$/i) && (i = !0);
					!1 === i && P("no known esriFieldTypeOID field detected in fields Array.  Please add an attribute field containing unique IDs to ensure the layer can be drawn correctly.")
				}
				this.options.timeField.start && this.options.timeField.end ? (this._startTimeIndex = new X, this._endTimeIndex = new X) : this.options.timeField && (this._timeIndex = new X), this._cache = {}, this._currentSnapshot = [], this._activeRequests = 0
			},
			onAdd: function(t) {
			...
			addFeature: function(t, i, s) {
				this._getMetadata(e.Util.bind(function(r, n) {
					...
					this.service.addFeature(t, e.Util.bind(function(e, r) {
			...
			createLayers: function(t) {
				for (var e = t.length - 1; e >= 0; e--) {
					...
					this._visibleZoom() && r && !this._map.hasLayer(r) && (this._map.addLayer(r), this.fire("addfeature", {
						...
						feature: i.feature
					}, !0), this._visibleZoom() && (!this.options.timeField || this.options.timeField && this._featureWithinTimeRange(s)) && this._map.addLayer(i))
				}
			},
